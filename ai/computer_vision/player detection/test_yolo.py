"""
Standalone video test: YOLO (best.pt) + tracking, NO homography, NO other
project files needed. Everything is in this one file on purpose, so file
placement/import path issues can't get in the way of just seeing whether
detection + tracking work on your real footage.

Usage:
    python quick_test_yolo_tracking.py --model best.pt --video 19.mp4 --max-frames 300

Uses ultralytics' own built-in ByteTrack (via tracker="bytetrack.yaml"),
not the custom bytetrack.py from the main project -- this script is meant
purely as a fast, dependency-free first look at your model + footage.
Once this looks good, switch to run_tracking_on_video.py (which uses the
project's own bytetrack.py and produces output ready for homography).
"""
...
def main():
    ...
    # Defaults filled in with your actual paths so this runs directly from
    # VS Code's Run button (Ctrl+F5) with no command-line arguments needed.
    # Still overridable from the terminal with --model / --video if you
    # want to point at a different file later.
    parser.add_argument(
        "--model", default=r"D:\SportsStrategyCoachAI\runs\runs\player_ball_v1-4\weights\best.pt",
        help="Path to best.pt",
    )
    parser.add_argument(
        "--video", default=r"D:\SportsStrategyCoachAI\SportsStrategyCoachAI\test.mp4",
        help="Path to match video",
    )
    parser.add_argument("--conf", type=float, default=0.25)
    parser.add_argument("--max-frames", type=int, default=None)
    parser.add_argument("--device", default=0,
                         help="0 for first GPU, 'cpu' for CPU. Default: 0 (GPU)")
    parser.add_argument("--imgsz", type=int, default=960,
                         help="Inference resolution. 960+ recommended for the ball class "
                              "(often <20px) per phase0_1_pipeline.py's training notes -- "
                              "the default YOLO 640 tends to miss it.")
